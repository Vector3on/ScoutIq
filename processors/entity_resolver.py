...
def execute_hf_fetch(prompt, model_id):
    ...
    # Accept both plain model names and full URLs
...
def resolve_reddit_leads_with_ai():
    ...
    # Provider pool (both plain names and full URLs work!)
    ai_providers = [
    ...
    with driver.session() as session:
        ...
        signals_result = session.run(
            "MATCH (s:Signal:Reddit) WHERE NOT (s)<-[:HAS_SIGNAL]-() RETURN s.title AS title, s.url AS url"
        )
        ...
        for i, signal in enumerate(unlinked_signals):
            ...
            prompt = (
                f'You are an expert entity resolver. Respond ONLY with a valid JSON object with one key: "best_match". '
                f'If you find a high-confidence match between the Reddit Title and a name from the Project List, the value must be the exact project name. Otherwise, the value must be "None".\n'
