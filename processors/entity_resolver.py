...
def execute_hf_fetch(prompt, model_id="google/gemma-7b-it"):
    ...
    try:
        ...
        if response.status_code == 503: # Model is loading
            ...
            print(f"    - WARN: Model '{model_id}' is loading. Waiting {wait_time} seconds...")
        ...
        # The model should respond with JSON, but we clean it just in case
        clean_json_str = generated_text[generated_text.find('{'):generated_text.rfind('}')+1]
...
def resolve_reddit_leads_with_ai():
    ...
    ai_providers = [
        ...
        {"name": "HF (Qwen2 7B)", "fetch": lambda p: execute_hf_fetch(p, "Qwen/Qwen2-7B-Instruct")},
    ]
    ...
    with driver.session() as session:
        ...
        for i, signal in enumerate(unlinked_signals):
            ...
            prompt = f'''You are an expert entity resolver. Respond ONLY with a valid JSON object with one key: "best_match". If you find a high-confidence match between the Reddit Title and a name from the Project List, the value must be the exact project name from the list. Otherwise, the value MUST be the string "None".
...
def run_resolver():
    ...
    print("  - Running Hugging Face AI Load Balancer / Entity Resolver...")
