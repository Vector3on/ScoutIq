...
class StartupSimEnv:
    ...
    def _initialize_state(self):
        self.state_keys: List[str] = [
            ...
            "hiring_market_strength", "competitive_pressure"
        ]
    ...
    def _apply_action_consequences(self, action: str):
        ...
        if action == "hire_engineer":
            ...
            if self.state['capital'] < 300000 and random.random() < self.state["investor_interest"] / 100:
                funding = random.uniform(50000, 250000)
                self.state["capital"] += funding
                if "secure_seed_round" not in self.unlocked_achievements:
                    self.unlocked_achievements.add("secure_seed_round")
        elif action == "refactor_codebase":
            self.state["technical_debt"] = max(0, self.state["technical_debt"] - 20)
            self.state["founder_burnout"] += 5
        elif action == "team_building_offsite":
            cost = 2000 * self.state["team_size"]
            if self.state["capital"] > cost:
                self.state["capital"] -= cost
                self.state["team_alignment"] = min(100, self.state["team_alignment"] + 15)
                self.state["founder_burnout"] = max(0, self.state["founder_burnout"] - 10)
        elif action == "rest":
    ...
    def _competitor_launch_event(self):
        ...
        if not self.state['done']:
            self.state["competitive_pressure"] = min(100, self.state["competitive_pressure"] + 40)
    ...
    def _economic_downturn_event(self):
        while not self.state['done']:
            ...
            if random.random() < 0.15:
                if not self.state['done']:
                    self.state["hiring_market_strength"] = max(10, self.state["hiring_market_strength"] - 30)
    ...
    def step(self, action: str) -> Tuple[Dict[str, Any], float, bool, Dict[str, Any]]:
        if self.state['done']:
            return self.get_state(), 0, self.state['done'], {}
