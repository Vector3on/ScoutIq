...
def run_diagnostic():
    """Runs a full diagnostic on the matching logic and prints the results."""
    print("="*50)
    print("BLOODHOUND MATCHING DIAGNOSTIC INITIATED")
    print("="*50)
    if not URI:
        print("FATAL: Neo4j credentials not found. Cannot run diagnostic.")
        return
    ...
    with driver.session() as session:
        ...
        signals_result = session.run("MATCH (s:Signal:Reddit) WHERE NOT (s)<-[:HAS_SIGNAL]-() RETURN s.title AS title")
        unlinked_signals = list(signals_result)

        if not projects or not unlinked_signals:
            print("No projects or unlinked signals found to diagnose.")
            driver.close()
            return
            
        print(f"Diagnosing {len(unlinked_signals)} unlinked Reddit signals against {len(projects)} projects...\n")

        for signal in unlinked_signals:
            title = signal["title"]
            potential_names = get_potential_names(title)
            
            print(f"--- For Reddit Post: '{title}'")
            if not potential_names:
                print("  - Could not extract any potential project names.")
                continue
