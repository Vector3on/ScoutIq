...
export function publicProgram(program, targetLimit = 16) {
  const compactRepoSignals = (signals) => signals ? {
    ...
    stars: signals.status === "ok" ? signals.stars ?? null : null,
    forks: signals.status === "ok" ? signals.forks ?? null : null,
    contributorsCount: signals.status === "ok" ? signals.contributorsCount ?? null : null,
    releases: signals.status === "ok" ? signals.releases ?? null : null,
    defaultBranch: signals.defaultBranch ?? null,
    languages: signals.languages ?? {},
    commits7d: signals.status === "ok" ? signals.commits7d ?? null : null,
    commits30d: signals.status === "ok" ? signals.commits30d ?? null : null,
    commits90d: signals.status === "ok" ? signals.commits90d ?? null : null,
    filesTouched90d: signals.status === "ok" ? signals.filesTouched90d ?? null : null,
    filesAdded90d: signals.status === "ok" ? signals.filesAdded90d ?? null : null,
    filesAdded90dList: (signals.filesAdded90dList ?? []).slice(0, 40),
    files90dTruncated: Boolean(signals.files90dTruncated),
    mergedPrs90d: signals.status === "ok" ? signals.mergedPrs90d ?? null : null,
    maxMergedPrAdditions90d: signals.status === "ok" ? signals.maxMergedPrAdditions90d ?? null : null,
    secTooling: signals.status === "ok" ? Boolean(signals.secTooling) : null,
    securityMd: signals.status === "ok" ? Boolean(signals.securityMd) : null,
    fuzzPath: signals.status === "ok" ? Boolean(signals.fuzzPath) : null,
    fuzzFunction: signals.status === "ok" ? Boolean(signals.fuzzFunction) : null,
    securityWorkflows: signals.securityWorkflows ?? [],
    advisories: signals.status === "ok" ? signals.advisories ?? null : null,
    advisoryCoverage: signals.advisoryCoverage ?? "unknown",
    trapHits: (signals.trapHits ?? []).slice(0, 10),
    trapTags: signals.trapTags ?? [],
    devKnown: signals.status === "ok" ? Boolean(signals.devKnown) : null,
    securityFixGated: signals.status === "ok" ? Boolean(signals.securityFixGated) : null,
    openRecentAdvisoryPathMatch: signals.status === "ok" ? Boolean(signals.openRecentAdvisoryPathMatch) : null,
    scanCoverage: signals.scanCoverage ?? null,
    trapScanStatus: signals.status === "ok" ? signals.trapScanStatus ?? "unknown" : "pending",
    scanErrors: (signals.scanErrors ?? []).slice(0, 5),
