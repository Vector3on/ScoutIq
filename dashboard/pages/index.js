...
// It connects to Neo4j, fetches the data, and injects it into our dashboard page.
export async function getStaticProps() {
  let neo4j;
  try {
    // This server-side dependency is required only within this function.
    // Wrapping it in a try-catch to satisfy the build tool.
...
const DashboardContent = ({ projects, error }) => {
    ...
    // Defensive coding: Ensure projects is an array before using .map or other array methods.
    const safeProjects = projects || [];

    const chartData = safeProjects.map(p => ({
        name: p.name ? p.name.split('/')[1] : 'Unknown',
        score: p.score || 0,
    ...
    const highestScore = totalProjects > 0 ? Math.max(...safeProjects.map(p => p.score || 0)) : 0;
                    ...
                    <table className="w-full text-left">
                        ...
                        <tbody>
                            {safeProjects.map((project, index) => (
                                <tr key={project.id || index} className="border-b border-gray-700 last:border-0 hover:bg-gamma-700/50 transition-colors">
                                    <td className="p-4 font-bold text-lg">{index + 1}</td>
                                    <td className="p-4 font-semibold">{project.name || 'N/A'}</td>
                                    <td className="p-4 text-gray-400 hidden md:table-cell">{project.description || 'N/A'}</td>
                                    <td className="p-4 font-bold text-indigo-400">{(project.score || 0).toFixed(2)}</td>
