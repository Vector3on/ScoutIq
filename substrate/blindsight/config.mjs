...
export function validateConfig(input, base = process.cwd()) {
  ...
  const llm = { enabled: false, endpoint: 'http://127.0.0.1:11434/v1/chat/completions', model: '', maxCalls: 4, ...input.llm };
  ...
  if (llm.enabled) {
    ...
    if (u.protocol !== 'https:' && !(u.protocol === 'http:' && ['localhost', '127.0.0.1', '[::1]'].includes(u.hostname))) throw Error('LLM requires HTTPS or local HTTP');
