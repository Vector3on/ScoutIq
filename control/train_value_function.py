...
# Assuming your ValueFunction class is in control/value_function.py
...
def train_value_function(data_file, output_path, resume_from_checkpoint, checkpoint_interval, epochs, batch_size, lr):
    ...
    for epoch in range(start_epoch, epochs):
        ...
        for batch_states, batch_rtg in dataloader:
            ...
            # Reshape tensors to treat every state in every sequence as a data point.
            # Original shape: [batch_size, seq_len, state_dim]
            num_sequences, seq_len, state_dim = batch_states.shape
            
            # Reshaped states: [batch_size * seq_len, state_dim]
            reshaped_states = batch_states.reshape(num_sequences * seq_len, state_dim)
            
            # Reshaped targets: [batch_size * seq_len]
            ...
            # The shapes should now match correctly for the loss calculation
            loss = loss_fn(predicted_values.squeeze(), reshaped_rtg)
            loss.backward()
            optimizer.step()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--output_path', type=str, default='value_function_v8.pth')
