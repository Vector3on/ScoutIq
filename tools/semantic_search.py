...
def find_similar_projects(target_project_name: str, top_k: int):
    ...
    try:
        ...
        print(f"FATAL: Could not load sentence-transformer model. Error: {e}")
        sys.exit(1)
    ...
    try:
        ...
        with driver.session() as session:
            ...
            if not all_projects:
                print("No projects found in the database. Ensure scrapers have run.")
                return
            ...
            if target_project_name not in project_names:
                ...
                # We do not exit with an error here, as the project might not be trending today.
                return
        ...
        print(f"FATAL: An error occurred during database operation or similarity search: {e}")
    finally:
