...
def update_project_with_velocity(tx, project_data):
    """
    This query is now much smarter.
    1. It MERGES the project node.
    ...
    ON MATCH SET
        // Calculate delta based on the old value BEFORE we overwrite it
        p.stars_delta_1d = $stars - p.stars, 
        // THEN update the stars to the new value
        p.stars = $stars, 
        p.language = $language,
    ...
    with driver.session() as session:
        for repo in repo_list:
            try:
                ...
                session.execute_write(update_project_with_velocity, project_data)
            except (AttributeError, ValueError, TypeError):
                continue
    
    driver.close()
    print(f"    - GitHub: Processed {len(repo_list)} projects, updating velocity.")
